...
char LoadalY(NNMATRIXxM *Y,int *num,char *BUFFER){
  FILE *fp;
  sprintf(BUFFER,"./KEYS/alY%05d.bin",*num);
  int i,j,k;
  ...
  for(i=0;i<NUM_M;i++){
	...
	Y->No[i].Matrix[j]._1byte[k]._8bit=fgetc(fp);
      }
    }
  }
  NNMATRIXxMtoTRANSPOSE(Y);
...
int main(){
  ...
  for((*i)=0;(*i)<NUM_L;(*i)++){
    ...
    SHA1_Update(&c1,Y,4*INTS_N*NUM_N*2*NUM_M);
  }
  SHA1_Final(sha1,&c1);
  free(Y);
  free(Message);
